   ...
   useEffect(() => {
      ...
      const checkVercelReadiness = async () => {
         try {
            ...
            const ready =
               hasLocalStorage && hasSessionStorage && hasResizeObserver && hasIntersectionObserver;

            setIsVercelReady(ready);
...
export function useLayoutCompatibility() {
   ...
   const compatibilityLevel: 'full' | 'basic' | 'none' = isVercelReady
      ? 'full'
      : isCompatible
        ? 'basic'
        : 'none';
...
export function withLayoutFeature<T extends Record<string, unknown>>(
   featureName: string,
   ...
   return function WithLayoutFeatureComponent(props: T) {
      ...
      if (process.env.NODE_ENV === 'development') {
         console.log(
            `Layout Feature "${featureName}" loaded with compatibility: ${compatibilityLevel}`
         );
      }
