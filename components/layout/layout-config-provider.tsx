...
interface LayoutConfigContextType {
  ...
  updateLayoutSettings: (settings: Partial<LayoutConfigState['settings']>) => void;
...
export function LayoutConfigProvider({ children }: { children: React.ReactNode }) {
  ...
  }, [sectionVisibility, leftSidebar.isOpen, rightSidebar.isOpen, isHydrated, setLeftSidebarOpen, setRightSidebarOpen]);
  ...
  const createNewView = useCallback((name: string, description?: string) => {
    if (!isHydrated) return '';
  ...
  const updateCurrentView = useCallback((updates: Partial<LayoutView>) => {
  ...
  const enhancedToggleSection = useCallback((section: LayoutSection) => {
    if (!isHydrated) return;
  ...
  }, [toggleSection, setLeftSidebarOpen, setRightSidebarOpen, leftSidebar.isOpen, rightSidebar.isOpen, isHydrated]);

  // Enhanced set section visibility function
  const enhancedSetSectionVisible = useCallback((section: LayoutSection, visible: boolean) => {
    if (!isHydrated) return;
  ...
  const getActiveTab = useCallback((section: LayoutSection) => {
  ...
  const addTabToSection = useCallback((section: LayoutSection, tab: Omit<ScreenTab, 'id'>) => {
  ...
  const removeTabFromSection = useCallback((section: LayoutSection, tabId: string) => {
    if (!isHydrated || !currentViewId) return;
...
export function withLayoutConfig<P extends object>(
